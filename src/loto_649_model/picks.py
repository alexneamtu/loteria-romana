...
def generate_picks(draws, count=2, rng=None, include_noroc: bool = True):
    ...
    best = pick_best_strategy(draws, rng=rng, include_noroc=include_noroc)

    if best == "neural":
        return generate_neural_lines(draws, count, rng=rng, include_noroc=include_noroc)
    if best == "frequency":
        freq = build_frequency(draws)
        return generate_frequency_lines(count, freq, rng=rng, include_noroc=include_noroc)
    return generate_random_lines(count, rng=rng, include_noroc=include_noroc)
